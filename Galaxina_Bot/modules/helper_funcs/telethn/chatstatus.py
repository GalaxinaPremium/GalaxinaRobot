from Galaxina_Bot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Galaxina_Bot import DRAGONS
...
async def galaxina_is_admin(chat_id: int):
    status = False
    galaxina = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if galaxina.id == user.id:
            status = True
            break
    return status
